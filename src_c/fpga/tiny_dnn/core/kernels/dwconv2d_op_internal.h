...
void dwconv2d_op_internal(const tensor_t &prev_out,
  ...
  dnn_addr[0] = 0; // init
  dnn_addr[0] = 2|16|256; // wwrite|backprop|dwconv
  ...
  /////////////////////////////////////////////////////////
  // Run
  dnn_addr[ 1] = ow*oh*1-1;  //fs//////
  dnn_addr[ 2] = ow*oh-1;    //ks
  dnn_addr[0] = 0;   // init
  dnn_addr[0] = 2|4|16|256; // wwrite|run|backprop|dwconv

  // DMA Buffer
  for(size_t o=0;o<id;o++){
    size_t ptr  =  o   *oh*ow;
    size_t ptr_ = (o/4)*oh*ow*4 + (o%4);
    for(size_t i=0;i<oh*ow;i++){
      conv16d.f = curr_delta[0][ptr+i];
      src_addr[ptr_+i*4] = conv16d.i;
    }
  }
  __asm__("DSB 15");
  ...
  dma_addr[0x00/4] = 1;
  dma_addr[0x18/4] = src_phys;
  dma_addr[0x28/4] = ((id+3)/4)*4*oh*ow*2;
  ...
  for (size_t sample = 0; sample < prev_out.size(); sample++){
    ...
    if(sample+1<prev_out.size()){
      for(size_t o=0;o<id;o++){
        ...
        size_t ptr_ = (o/4)*oh*ow*4 + (o%4);
        for(size_t i=0;i<oh*ow;i++){
          conv16d.f = curr_delta[sample+1][ptr+i];
          src_addr[ptr_+i*4] = conv16d.i;
        }
      }
      __asm__("DSB 15");
      ...
      dnn_addr[0] = 2|4|16|64|256; // wwrite|run|backprop|last|dwconv
    }
