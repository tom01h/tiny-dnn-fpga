...
#include "tiny_dnn/core/kernels/dwconv2d_grad_op.h"
#include "tiny_dnn/core/kernels/dwconv2d_op.h"
 ...
 **/
class dwconvolutional_layer : public layer {
 public:
  /**
   * constructing DW convolutional layer
   *
   ...
   **/
  dwconvolutional_layer(size_t in_width,
                      size_t in_height,
                      size_t window_size,
                      size_t in_channels,
    ...
    : dwconvolutional_layer(in_width,
                          in_height,
                          window_size,
                          window_size,
                          in_channels,
   ...
   **/
   dwconvolutional_layer(size_t in_width,
    ...
    dwconv_set_params(shape3d(in_width, in_height, in_channels), window_width,
                    window_height, pad_type, has_bias, w_stride,
  ...
  // move constructor
  dwconvolutional_layer(dwconvolutional_layer &&other)  // NOLINT
  ...
  std::string layer_type() const override { return std::string("dwconv"); }
 ...
 private:
  ...
  void dwconv_set_params(
    ...
    params_.out = shape3d(
      ...
      conv_out_length(in.height_, w_height, h_stride, h_dilation, ptype), in.depth_);
    params_.weight     = shape3d(w_width, w_height, in.depth_);
    ...
    padding_op_ = core::DWConv2dPadding(params_);
  }
  ...
  void init_backend(const core::backend_t backend_type) {
    ...
    if (backend_type == core::backend_t::internal) {
      kernel_fwd_.reset(new DWConv2dOp(ctx));
      kernel_back_.reset(new DWConv2dGradOp(ctx));
      return;
    } else {
 ...
 private:
  ...
  core::dwconv_params params_;

  /* Padding operation */
  core::DWConv2dPadding padding_op_;
